...
public class NoxesiumInitializer {
    ...
    public void initialize() {
        ...
        NoxesiumNetworking.send(new ServerboundHandshakePacket(ids));
    }
    ...
    private void handle(ClientboundHandshakeAcknowledgePacket packet) {
        ...
        var fabricApi = NoxesiumFabricApi.getInstance();
        ...
        for (var pair : packet.entrypoints().entrySet()) {
            ...
            if (entrypoint instanceof FabricNoxesiumEntrypoint fabricEntrypoint) {
                fabricEntrypoint.getPacketCollections().forEach(fabricApi::registerPackets);
            }
        }

        // Inform the server about the handshake success
        NoxesiumNetworking.send(new ServerboundHandshakeAcknowledgePacket(entrypoints));
    }
    ...
    public void uninitialize() {
        ...
        NoxesiumFabricApi.getInstance().unregisterAll();
